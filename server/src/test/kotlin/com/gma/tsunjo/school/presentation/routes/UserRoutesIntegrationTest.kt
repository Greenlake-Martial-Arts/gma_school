...
class UserRoutesIntegrationTest {
    ...
    @Test
    fun `GET users returns 200`() = testApplication {
        val response = client.get("/users")
        assertEquals(HttpStatusCode.OK, response.status)
    }

    @Test
    fun `GET users active returns 200`() = testApplication {
        val response = client.get("/users/active")
        assertEquals(HttpStatusCode.OK, response.status)
    }

    @Test
    fun `GET users by invalid id returns 400`() = testApplication {
    ...
    @Test
    fun `POST users with valid data returns 201 or 409`() = testApplication {
    ...
    @Test
    fun `PUT users with invalid id returns 400`() = testApplication {
    ...
    @Test
    fun `PATCH users deactivate with invalid id returns 400`() = testApplication {
    ...
    @Test
    fun `POST auth login with invalid credentials returns 401`() = testApplication {
